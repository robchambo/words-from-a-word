...
## v2-baseline reconciliation (read before executing)

This plan was authored against `main`'s pre-median-calibration baseline. When reconciled against v2 (= median-calibration code + main's docs), several subtasks are already done by commit `1c46903` ("Add Android release signing, launcher icon, beta version") and must be **skipped** to avoid clobbering working configuration.

**Already done on v2 — do not redo:**

| Subtask | State on v2 | Action |
|---|---|---|
| `flutter_launcher_icons` dev-dep in `pubspec.yaml` | Present at `^0.14.1` (plan specified `^0.13.1`) | Keep v2's version; do not downgrade. |
| `flutter_launcher_icons` generator config | Present; points at `assets/icon/app_icon.png` | Keep v2's path. Plan's `assets/branding/icon_source.png` path is obsolete. |
| Android adaptive launcher icon | Generated from Kat's Soviet-notebook source; cream `#FFFEF0` background | Skip Task 2 Android regeneration. |
| Android `android:label` → "Слова из Слова" (Cyrillic) | Done | Skip the corresponding Modify row under File Structure. |
| Android release signing (`android/key.properties` + `upload-keystore.jks`, gitignored; debug fallback) | Done | Not a task in this plan originally, but flag as complete for the release manager. |
| `pubspec.yaml` version bumped to `1.1.0-beta.1+2` | Done | Plan's final tag should be `v1.1.0` — bump again at release time. |

**Still needed (execute as written, with the path substitutions noted):**

- **Task 1** — Add only `flutter_native_splash: ^2.4.0` as a dev-dep; the `flutter_launcher_icons` line is already present. Add the `flutter_native_splash:` generator block. Do **not** re-add or modify the `flutter_launcher_icons:` block (its `image_path` is already `assets/icon/app_icon.png`, not `assets/branding/icon_source.png`).
- **Task 2** — iOS side only. The iOS `AppIcon.appiconset` has not been regenerated from Kat's source. Run `dart run flutter_launcher_icons` on a macOS host with the current config (which already has `ios: true` if set; verify in v2's `pubspec.yaml` and add if missing). Splash artwork (`assets/branding/splash_source.png`) is still needed — Android splash will be generated by `flutter_native_splash` for the first time.
- **Task 9 (CI)** — `.github/workflows/ci.yml` **already exists** on v2 (created by Stage 2 with `flutter analyze` + `flutter test` on PRs/pushes to `v2` and `main`). Phase 9 Task 9 must **amend** the existing file, not create a new one: add the `dart run tool/validate_levels.dart` step (after Phase 7 lands) and the `build-android` job that uploads a debug APK. The optional `.github/PULL_REQUEST_TEMPLATE.md` is **also already created** by Stage 2 — skip Task 9 Step 2 or enhance the existing template rather than overwriting it.
- **Tasks 3–8, 10, 11** — Execute unchanged. None of this work exists on v2.

**Path substitution to apply throughout the plan:** wherever the plan says `assets/branding/icon_source.png` or `icon_foreground.png`, v2 uses `assets/icon/app_icon.png`. Splash artwork path `assets/branding/splash_source.png` is still the correct target (no v2 conflict).
...
**Tech Stack:** Dart 3.11, Flutter, `flutter_launcher_icons ^0.14.1` (already on v2), `flutter_native_splash ^2.4.0` (still to add), GitHub Actions.
