...
**Resolved 2026-04-16 in Phase 0 of the v1.1 roadmap.** See commit history, GDD §5.1, §5.7, §9.1, and `DECISIONS.md` D10.

- ✅ **Level JSON schema has no `id` field.** GDD §5.1 example corrected — the `"id": 1` line removed. Level ID is derived from `LevelLoader.generateLevel(levelNumber)`. Decision logged in D10.
- ✅ **End-of-library silently loops.** Confirmed v1.0 bug (not a feature). GDD §5.7 updated to describe the v1.1 fix: explicit library-complete screen with free-mode replay. Code change scheduled for Phase 3. Decision logged in D10.
