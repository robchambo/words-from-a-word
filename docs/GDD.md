...
Each level's ID is derived from its array position by `LevelLoader.generateLevel(levelNumber)` — the JSON does not carry an explicit `id` field (see `DECISIONS.md` D10).

`LevelLoader` validates every word at load time using `GameEngine.canFormWord()` and caps required words at 12 per level (D5). Words beyond the cap are silently dropped, not demoted to bonus.
...
- Level JSON does not carry an explicit `id` field; ID is derived from array position (D10).
...
Schema per §5.1 (existing) plus `difficulty: 1..5` (§5.2, new). `LevelLoader` validates all words against the source word via `GameEngine.canFormWord()` at load, caps required words at 12 (D5), drops overflow silently.
