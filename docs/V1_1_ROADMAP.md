...
- Fix the end-of-library behaviour. `LevelLoader.generateLevel` currently wraps silently via `(levelNumber - 1) % defs.length`. Replace with an explicit "library complete" result and decide UX (loop? show replay screen? show "more coming" message?). Document the choice in GDD §5.7 and DECISIONS.md (new entry D17).
